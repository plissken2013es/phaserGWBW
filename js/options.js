    ...
    createOptionsFor: function(opt) {
        ...
        if (opt.infected) {
            for (var i=0; i < opt.infections.length; i++) {
                var txt = this.gameLink.add.bitmapText(this.gameLink.input.mousePointer.x - 20, this.gameLink.input.mousePointer.y - 20 + i * 18, "minecraft", opt.infections[i].text, 10);
            ...
            for (var i=0; i < opt.options.length; i++) {
                var txt = this.gameLink.add.bitmapText(this.gameLink.input.mousePointer.x - 20, this.gameLink.input.mousePointer.y - 20 + i * 18, "minecraft", opt.options[i].text, 10);
                txt.smoothed = false;
                if (txt.x + txt.textWidth > this.gameLink.world.width) txt.x -= Math.floor(txt.textWidth/2);
